...
- [ ] Registration page at `/register` (requires authentication — redirect to `/auth/signup` if not logged in)
- [ ] Form fields: business name (required), street address (required), city (required), state (required), zip (required), category (required, open text input with autocomplete from existing categories), ownership type (required, 6-tier selector with plain-language descriptions), story/about (optional, free text)
- [ ] Category field is open text, NOT a fixed dropdown (per ADR-2)
- [ ] Ownership selector shows all 6 tiers with brief descriptions
- [ ] Selecting "Mission-driven" reveals additional field for certification type / rationale
- [ ] On submit: address is geocoded to lat/lng via Mapbox Geocoding API
- [ ] Slug is generated from business name (lowercase, hyphenated, collision-safe)
- [ ] Business record is created in Supabase with all fields
- [ ] After creation, user is redirected to their listing page (`/business/{slug}`)
- [ ] Invalid address (geocoding fails): clear error message asking user to check address
- [ ] Duplicate address: warn but allow submission
