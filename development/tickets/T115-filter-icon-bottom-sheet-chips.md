...
**Tests.** 179 across the T115 set, all green: `filters.test.ts` (37), `ExploreFilterSheet.test.tsx` (26), `ExploreSearchBar.test.tsx` (14), `ActiveFilterChips.test.tsx` (7), `useScrollRestoration.test.ts` (6), `origin.test.ts` (4), `query.test.ts` (7), `items.test.ts` (19), `ExplorePage.test.tsx` (34), plus T114's `KindFilterPills` and `kinds` suites untouched. Full suite matches the `main` baseline exactly (10 failed / 1319 passed; the ten are pre-existing flaky CI-conformance and auth specs, identical on both branches when run in isolation). Build passes.
