...
- [ ] Component file `web/components/sell/SellWalkthrough.tsx` composing `<MultiStepComposer>` with five steps (per F036 scenario body):
  ...
  4. **Locality claim (Tier 0)** — ~~ZIP input → writes a `member_business_jurisdictions` row with `verification_source='self_attested'` if filled.~~ *(Patched: review-F036.md determined F036 does NOT need the `member_business_jurisdictions` substrate — it ships with F037. Implementation chose UI-only at b1; this step collects ZIP for display only, no substrate write.)* Marked `isOptional=true` so the Multi-step composer renders the `[Skip]` link. F037 owns the badge UI and the substrate write.
