...
**Description:** Members see a locality-aware feed of nearby Items — gatherings, Wonders, Seller updates, producer bulletins (T2+) — anchored on the Member's locality but never Location-message-scoped.
...
As a Member, I want to open the app and see what's happening near me — gatherings to attend, Sellers to follow, Wonders to engage with — so I can discover the activity in my locality without searching for it.
...
- Card types at b1:
  ...
  - **Seller update** (text post from a Member who has ≥1 active kind='business' Group membership OR at least one product/service Item).
...
- Filter by card type (Gatherings / Wonders / Seller updates) — pill row at top of feed.
...
- "Sell" CTA visible on Home for Members with no active kind='business' Group membership and no kind='product'/'service' Items (per ADR-12 SUPERSEDED 2026-05-12).
...
- [Member Profile](member-profile.md) — destination when a Seller update card is tapped.
