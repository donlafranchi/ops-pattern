...
> Companion to [`principles.md`](principles.md). How the platform earns is constitutional, because revenue shape determines whether the platform can honor the mandate. This doc carries the working hypothesis.
...
**The platform will not raise venture capital.** VC funding optimizes for exit, and an exit-aligned platform is misaligned with the load-bearing purpose (Members materially better off; Flourishing as the north star). VC pressure on growth metrics also creates structural pressure toward the engagement-optimization failure modes the platform exists to refuse. The platform earns its revenue from product use, not from raising rounds.
...
- **Sponsored federation handoffs (long-horizon).** When the platform spawns federated infrastructure (banking, insurance, BI), the federation partners may pay placement-shaped fees that do not compromise Member-side experience.
