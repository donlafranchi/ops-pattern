...
This is the framework every Project policy — written or proposed — passes through before it becomes an enforced behavior in the platform. It is not a list of policies; it is the test the policies have to pass and the posture they have to start from.
...
## The accountable-participation commitments (design intent, structural)

Three commitments encode the platform's response to the anonymous-complaint-feed failure mode. Location-scoped anonymous feeds breed hostility because no one is accountable — there is no real identity behind a complaint, so people act with impunity. The platform's answer is that every action is tied to a real identity. These commitments shape current surface decisions and constrain future ones; they are not categorical "never"s, with the exception of the privacy commitment enforced by owner-only RLS on `member_place_interests` and `member_saved_searches`, which stays as a hard architectural floor.
...
**The hard architectural floor — owner-only RLS on private Member geography substrates.** Per the 2026-05-23 substrate split, this is a privacy commitment, not an anti-feed commitment, and it stays as an absolute. No peer Member, anon visitor, or query path can `SELECT` another Member's `member_place_interests` or `member_saved_searches` rows under any condition. If a future surface needs an aggregate ("how many Members opted into Sacramento as a place-interest?"), it lands as a named SECURITY DEFINER function whose output is counts or booleans — never per-Member identity. A future engineer who tries to JOIN against these tables to build a send-to list is blocked by the database itself, not by code review. The public counterpart (`member_business_jurisdictions`, the seller-locality claim per `business-jurisdiction.md`) is intentionally public by design — sellers declare locality as a public claim, not a private affinity. The substrate split is what lets each table carry the right RLS posture without the prior fused-affinity-table contradiction.

Surfaces that touch this design space pass through the three-filter test in the system spec. The privacy floor is non-negotiable; everything above it is a design conversation.
...
|---|---|---|
| Policy posture | Accepted | The three-filter test (helpful? harmless? abuse-resistant?). The opt-out default (protective stance is default; Member opts in to relax). The "Policy posture" requirement on every system spec touching privacy, revenue, monetary flow, or data sharing. The accountable-participation design intent (messaging item-or-group at b1; Location-scoped surfaces designed carefully when they appear; push-back-on-complaint-only behavior; fix-it path offered not forced). The owner-only-RLS commitment on `member_place_interests` and `member_saved_searches` stays as a hard architectural floor inside this scope. Concrete opt-in shapes: `agent-assistance.md` (k-anonymity floor N≥10; cross-Member sharing); `agent-assistance.md` (platform-mediated payment with capped cut); `agent-assistance.md` (`recurring_payment` scope with required caps + allowlist + expiry; `bounded_purchase` scope with required caps + recipient_scope + category_scope + reversibility window per `payments.md`). |

This file also *encodes* (but does not own) the Member↔Geography substrate split: the accountable-participation commitment §1 above is upgraded from a policy commitment to a structural RLS enforcement on `member_place_interests` and `member_saved_searches`. The substrate split lives cross-cutting in [`../../playbooks/PLATFORM-PATTERNS.md`](../../playbooks/PLATFORM-PATTERNS.md); its data-model home is [`member.md`](../systems/member.md); its locality-derivation surface is [`business-jurisdiction.md`](../systems/business-jurisdiction.md) (read by `groups.md`). The prior fused-affinity table was superseded by the substrate split on 2026-05-23.
