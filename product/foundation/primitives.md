...
- **Person ↔ Location** — multi-belonging via `member_location_affinities`. A Member declares affinities of kind `lives` / `works` / `plays` / `visits` / `follows` / `liked` for any number of Locations. The single `home_location_id` column is the locality default (per ADR-4); the affinity table holds the rest. **Affinities never grant addressability** — no DM, feed, or wall is scoped to a Location (per the no-Location-messaging commitment in [`policy-framework.md`](policy-framework.md)). The structural anti-Nextdoor commitment lives in messaging scope and complaint downvote/removal, not in the absence of Person-Location relationships.
