...
The rail is where the platform's single categorical "Never" — wealth circulation over extraction (owned by [`../foundation/principles.md`](../foundation/principles.md); operational rule in [`../../playbooks/DECISION-PATTERNS.md`](../../playbooks/DECISION-PATTERNS.md)) — becomes load-bearing architecture: a downstream agent facing a rail / partner / fee choice refuses any option that routes value out of the community absent an explicit time-bounded justification (cards at b2 with friction is the canonical justified exception).
...
>
> > **Intent:** Two failure modes prevented: (a) the platform charges extractive fees (Stripe-style cuts compounding to shareholders); AND (b) the platform declines viable non-extractive revenue and starves itself out. The job of §9 is to collect fees that help the platform AND route excess back to communities — the earn-before-extract fee philosophy and the Yelp / Etsy / Stripe comparison are owned by [`../foundation/monetization.md`](../foundation/monetization.md). A downstream proposal must show: is this fee shape extractive (refused) or non-extractive with mission-aligned revenue routing (admissible)?
