...
Per ADR-12 (which supersedes ADR-3), the Operation declaration is itself the explicit "Become a Maker" CTA — the original ADR-3's "no separate Maker onboarding" framing is rejected. The Member-Operations integration here remains the same: standing-tier is reached when the Member has ≥1 active Operation, regardless of whether the Maker-mode toggle (`maker_mode_enabled`) is on or off.
