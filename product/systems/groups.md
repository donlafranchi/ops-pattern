...
This spec also *encodes* (but does not own) ADR-21 (Member↔Geography substrate split): the locality-promotion derivation in the **Locality and promotion** section above is the load-bearing surface — it reads `member_business_jurisdictions` via `public.zip_is_proximal_to_location()` as the first signal, per ADR-21. The owner-only-RLS posture for private Member geography substrates (`member_place_interests`, `member_saved_searches` in `member.md`) belongs to ADR-21 outright. ADR-21 lives cross-cutting in [`../../planning/DECISIONS.md`](../../planning/DECISIONS.md).
