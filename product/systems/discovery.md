...
- **`member_place_interests`** (per [`member.md`](member.md)) — the Member's awareness scope (one `primary_home` Place + up to 5 `secondary` Places). The candidate set is bounded by Items whose attached Location falls inside the Member's `metro_polygons` overlay via `ST_Intersects`. Default scope is *metro* — a Member with `primary_home=Oak Park` sees Items across the Sacramento-Roseville metro (Oak Park, Folsom, Roseville, Davis) out of the box. City and neighborhood granularity controls are deferred to b2.

  > **Intent (Ratified 2026-09-02; supersedes Intent Ratified 2026-05-23 — see [memo-0026](../../playbooks/memos/memo-0026-metro-default-feed-depth.md)):** *Metro* is the default depth because at b1 scale, more discovery out of the box beats a tighter locality signal — a city-scoped feed in a city with 3 listings is useless, while metro scope gives critical mass from day one. Members who want a narrower view can filter by city or neighborhood when b2 granularity controls ship. State-never depth is excluded by the same reasoning carried further.
...
Per PM direction 2026-05-23: **search is probably the platform's most helpful tool.** The current saved-search substrate (`member_saved_searches`, per `member.md`) is a *starting point*. The larger vision spans the tiers below:
