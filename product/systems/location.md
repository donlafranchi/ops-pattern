---
id: what-location
purpose: Permanent / recurring-temporary / area places; accountable participation encoded.
layer: what
status: active
---
...
- **Not a complaint surface.** The accountable-participation commitment (see [`policy.md`](../foundation/policy.md)) takes its structural form here: when messaging surfaces ship (b2+), they are scoped to Items or Groups only — never to Locations. There is no Location wall, no Location feed, no Location DM. Complaint-style content in any feed surface can be downvoted and removed from circulation. The platform's affordance for "I have a problem with this place" is to create an Item — a Wonder ("would folks be into fixing the broken playground?") or an Initiative ("let's organize the playground rebuild") — and lead the fix.
...
`members.home_location_id` is **reserved as** the single locality default — the Location used for "near me" defaults when the Member hasn't picked a different scope. It is not a public attribute. Cross-Place secondary interests live on `member_place_interests`.

*Corrected 2026-09-03:* the spec previously described this as live, with the `primary_home` `member_place_interests` row derived *from* `home_location_id` at onboarding. **The column is dead** — never populated, never read; the `member.locality.set` handler that would write it does not exist (migration `031_metro_polygons.sql` header; inline notes in `place-interest-add.ts` / `place-interest-remove.ts`). The dependency runs the other way in shipped code: the Member picks a `primary_home` Place, and `resolve_home_metro()` derives `members.home_metro_id` from that Place's centroid — the live locality-default path. **Item location does not read any of this:** a Member enters an Item's location at creation (address, neighborhood, or Online) per [`../../planning/backlog/decision-surfaces.md`](../../planning/backlog/decision-surfaces.md) § Location is entered at creation, which is what retired the earlier creator-inheritance design and with it any dependency on this column.

**No `location_memberships` table; no per-Location follow row.** Memberships are a Group concept. Subscriptions are a saved-search concept. The semantic distinction (affinity vs. membership vs. subscription) is what keeps the platform from sliding into the anonymous-complaint-feed pattern, but the structural prevention lives in the messaging-scope and complaint-downvote commitments above — not in the absence of these tables.
...
- **Item** — `item_locations` (per `item.md`) attaches Items to Locations with a per-attachment schedule. An Item can attach to multiple Locations (the Quarterly Dip Vendor's appearances at three different markets); a Location can host many Items across many Members.
...
- **Messaging (forward-looking, b2+)** — when DM and other messaging surfaces ship, **none are scoped to Locations.** Messaging is item-or-group only per the no-Location-messaging commitment in `policy.md`. There is no Location DM channel, no Location wall, no Location-scoped feed. This is the structural prevention against the anonymous-complaint-feed pattern.
...
This spec also *encodes* (but does not own) bottom-anchored UI surfaces, the locality default via `members.home_location_id`, markets-are-Gathering-Items (not Locations of a special kind), action handlers + audit fields, the action layer, the accountable-participation messaging-scope (item-or-group-only), and locality-scoped URLs (Locations anchor to Places via `place_id`, with place-scoped slugs and URLs; the Places substrate itself lives in [`places.md`](places.md)). Those live cross-cutting in `DECISIONS.md`.
