...
- **Saved searches scoped to a Location.** The "Follow this venue" UI affordance on a Location page creates a `member_saved_searches` row with `location_id` set — the Member subscribes to filtered streams of Items posted at that Location. Per ADR-21; see [`member.md`](member.md).

`members.home_location_id` stays as the single locality default (per ADR-4) — the Location used for "near me" defaults when the Member hasn't picked a different scope. It is not a public attribute. A `member_place_interests` row of `scope_kind='primary_home'` is derived from `home_location_id`'s `place_id` at onboarding; that row drives the community-awareness feed in `discovery.md`. Cross-Place secondary interests live on the same table.

**No `location_memberships` table; no per-Location follow row.** Memberships are a Group concept. Subscriptions are a saved-search concept (per ADR-21). The semantic distinction (affinity vs. membership vs. subscription) is what keeps the platform from sliding into Nextdoor's pattern, but the structural prevention lives in the messaging-scope and complaint-downvote commitments above — not in the absence of these tables.
...
- **Discovery** — `discoverable_items` materialized view JOINs to nearest Location for every row; the view's locality column is populated from `locations.geography`. The Venue page pattern (per `community-platform.md`) is the Location's user-facing surface — header layout, sections, primary "Host something here" CTA. Per ADR-21, the community-awareness feed reads `member_place_interests` × `member_interests` (no per-Location follow row); the b2 saved-search fan-out reads `member_saved_searches` joined to new `item_locations` writes.
