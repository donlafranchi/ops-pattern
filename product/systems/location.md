...
**Companion specs:** [`primitives.md`](../foundation/primitives.md) · [`item.md`](item.md) · [`member.md`](member.md) · [`groups.md`](groups.md) · [`policy-framework.md`](../foundation/policy-framework.md) · [`discovery.md`](discovery.md)
...
- **Messaging (forward-looking, b2+)** — when DM and other messaging surfaces ship, **none are scoped to Locations.** Messaging is item-or-group only per the no-Location-messaging commitment in `policy-framework.md`. There is no Location DM channel, no Location wall, no Location-scoped feed. This is the structural prevention against Nextdoor's complaint-attractor pattern.
