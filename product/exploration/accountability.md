...
- **Related platform-level commitments to honor at spec time:** the anti-Nextdoor design intent in `policy.md`, the "no ranking of people" corollary in `principles.md` Part 2, and the action-layer audit trail in `action-layer.md` (every report is an action; every admin verification is an action; both flow through the named handlers).
