...
- No new RLS policies on `groups` or `group_memberships`. The suggestion surface reads from `item_saves` (Member's own rows + aggregate counts via SECURITY DEFINER, matching the owner-only-RLS + named-function-aggregates pattern documented in `member.md` § Structural privacy — the standard posture for private substrates under ADR-21). The Group, once created, is governed by the existing `groups.md` policies.
