...
**Accepted risk — metro boundaries are administrative and lives cross them.** Right for the far case (home here, cabin three hours away). **Wrong for the near case:** a Member living twenty minutes from a metro line, with home and work hoods on either side, is forced to switch between halves of one ordinary life — and edge-dwelling Members are exactly who a neighbours product should serve best. Recorded as an accepted risk with a known bad case, not a solved problem. Mitigation to consider later: union adjacent metros when a Member's own hoods straddle them. Separately, `members.home_metro_id` is **null outside every seeded CSA** (migration `031`, rural fallback), so rural Members have no metro to be the vantage point; the existing radius fallback is the working answer. Full entry: [`../../planning/backlog/decision-surfaces.md`](../../planning/backlog/decision-surfaces.md) § Metro is the vantage point.
...
**Open:** when the Member is asked (signup vs. first creation); what determines the default metro when hoods span several and whether one can be primary; what the active metro does when a hood is added in a new one; whether the geocode-once step is what assigns a hood to its metro; whether hoods are capped. `decision-surfaces.md` § Open questions 4–8.
