...
**The field is required, and pre-filled (Ratified 2026-09-03).** The Member is asked for their neighborhood **once**; it pre-fills the location field on every creation and they override it per Item. That is what resolves required-field friction against § Create is first class — the field arrives already filled in, so confirming costs nothing and overriding is a choice rather than a chore. Overriding to a full address is how a specific venue is handled: neighborhood-as-default does not mean neighborhood-only precision. **Build note — this is a default, not inheritance:** the neighborhood is **copied onto the Item at creation and owned by the Item**; the Item must never read through to the Member's profile at display time. A Member who moves and updates their neighborhood must not silently relocate their past Items, and a per-Item override is permanent to that Item. Copy the value, do not store a reference. **Open:** whether the Member is asked at signup or at first creation — `decision-surfaces.md` § Open questions 4.
