...
- Measured contrast `--color-fg-muted` on `--color-surface` is **4.93:1** against the tokens `globals.css` actually ships (`#6b6b6b` on `#f7f6f2`) — clears AA for text and clears the 3:1 bar for graphical objects. Do not lighten either token in this pairing without re-measuring.
  - Measured live, not from the table above: the § Color role tokens in this document have drifted from `globals.css` (documented `#717171` / `#F7F7F7` / `#EBEBEB` / `#2D2D2D`; shipped `#6b6b6b` / `#f7f6f2` / `#e5e3dd` / `#1a1a1a`). The drift predates this recipe and is the open question in `planning/backlog/decision-charcoal-ramp-migration.md`. Against the documented values the same pairing measures 4.56:1, so this recipe clears AA either way — but measure against `globals.css`, not against the table.
