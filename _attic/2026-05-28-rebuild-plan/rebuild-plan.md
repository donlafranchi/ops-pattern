...
- **ADR-4** (locality default = geolocate then city-pick, mutable) — retained. Per ADR-21 (2026-05-23), multi-Location belonging via the six-kind `member_location_affinities` enum is **retired**; the three purpose-owned substrates are `member_business_jurisdictions` (jurisdiction), `member_place_interests` (community awareness scope), and `member_saved_searches` (generalized follow / interest-in-a-Place).
...
7. **Member↔Geography substrates (per ADR-21).** Three purpose-owned tables at b1: `member_place_interests` (one `primary_home` + up to 5 `secondary` Places per Member; private, owner-only RLS), `member_saved_searches` (generalized labeled-filter shape; absorbs "follow this venue"; private; surface and fan-out worker ship at b2), and `member_business_jurisdictions` (seller-only, public, Tier 0 self-attested at b1; Tier 1 community-attested at b2+ with sibling `member_business_jurisdiction_attestations` table; Tier 2 document-supported at b2+/b3). The retired `member_location_affinities` table and its three named SECURITY DEFINER functions (`count_likes_for_location`, `count_followers_for_location`, `member_is_local_to_location`) **do not ship** — never create them.
8. **Verification-ladder reshape (ADR-21 ratification 2026-05-23).** Both "Locally Owned" (jurisdiction) and "Locally Made" (provenance) share the same three-tier ladder: Tier 0 self-attested (b1) → Tier 1 **community-attested** (b2+, peer pressure for the greater good) → Tier 2 document-supported (b2+/b3). The original SOS API integration for jurisdiction Tier 1 is retired — community-attestation is the platform-internal path. The `verification_source` enum on `member_business_jurisdictions` ships at b1 as `('self_attested','community_attested','document_upload')` (b1 only writes `self_attested`; the other values are reserved). The `made_at_verification_source` enum on `items` mirrors: `('none','self_attested','community_attested','document_supported')`. Attestation substrate tables (`member_business_jurisdiction_attestations`, `item_made_at_attestations`) and the threshold worker do **not** ship at b1.
...
- `007i_member_place_interests.sql` (per ADR-21) — community-awareness scope. Composite PK on `(member_id, place_id, scope_kind)`; `scope_kind` enum (`primary_home`, `secondary`). Unique partial index `(member_id) where scope_kind='primary_home' and removed_at is null` enforces exactly one active primary_home per Member. Index on `(member_id, scope_kind) where removed_at is null` for the Member's own surfaces. **Owner-only RLS** per ADR-21. Action-layer guard enforces the secondary cap (≤5 per Member; tuneable without migration).
- `007ii_member_saved_searches.sql` (per ADR-21) — generalized follow / interest-in-a-Place. `id` UUID PK, `member_id`, `label` (1–80 chars), nullable `place_id` and `location_id`, `interest_tags text[]`, `item_kinds text[]`, soft-delete. Check constraint: at least one of `place_id`, `location_id`, or non-empty `interest_tags` must be set. Indexes on `(member_id) where removed_at is null`, `(place_id) where place_id is not null and removed_at is null`, `(location_id) where location_id is not null and removed_at is null` to drive the b2 fan-out worker. **Owner-only RLS** per ADR-21.
- **Retired — do not create:** `007i_member_location_affinities.sql`. The six-kind `member_location_affinities` table and its three named SECURITY DEFINER functions (`count_likes_for_location`, `count_followers_for_location`, `member_is_local_to_location`) are retired per ADR-21 (2026-05-23). The locally-owned derivation now reads `member_business_jurisdictions` via `public.zip_is_proximal_to_location()` exclusively (per `business-jurisdiction.md`).
- `007j_member_events.sql` — Member event log, partitioned monthly per ADR-10. Carries `acting_member_id NOT NULL` + `via_delegation_id` per ADR-6. Partition rotation routine (pg_partman or pg_cron) installed here. Event kinds at b1: `member.created`, `member.profile_updated`, `member.handle_changed` (T2 surface; reserved), `member.home_location_set`, `member.privacy_changed`, `member.followed` / `member.unfollowed`, `member.place_interest_added` / `member.place_interest_removed` / `member.place_interest_promoted` / `member.place_interest_demoted` (per ADR-21), `member.saved_search.created` / `member.saved_search.updated` / `member.saved_search.removed` (per ADR-21), `member.interest_added` / `member.interest_removed`, `member.delegation_granted` / `member.delegation_revoked`, `member.deleted` / `member.restored`, `member.export_requested`, `member.purge_executed`. **Retired event kinds (do not ship):** `member.location_affinity_added` / `member.location_affinity_removed` / `member.maker_mode_changed`.
...
- `008c_location_areas.sql` — child for kind='area': `polygon geography(Polygon, 4326) NOT NULL`, `area_kind` enum (`service_radius`, `neighborhood`, `custom`) — **`city` and `region` dropped per ADR-20** (recognized civic geography lives in the Places primitive, not as Member-declared area Locations). `radius_meters int` (nullable, populated for circular areas). On insert/update, a trigger computes the polygon's centroid and writes it to the spine's `geography` column so proximity queries work uniformly across kinds.
- `008d_location_events.sql` — Location event log, partitioned monthly, audit fields. Event kinds at b1: `location.created`, `location.updated`, `location.moved`, `location.polygon_updated`, `location.hours_updated`, `location.deleted`, `location.restored`. Reserved at b1, surfaces at T2: `location.claim_requested`, `location.claim_resolved`, `location.contributor_added`. **Retired per ADR-21:** `location.followed` / `location.unfollowed` — Location-follow is a saved-search shape (`member.saved_search.*` events), not a Location-level event.

**Item surface (009 series — spine + 4 children + relations + events):**

- `009_items.sql` — spine: `id`, `member_id`, `kind` enum (`product`, `service`, `gathering`, `wonder`; `offer`, `ask`, `initiative` reserved). **No `cooperative_cohort` value** — dropped per the Groups ratification. `title`, `description`, `state` enum (`active`, `fulfilled`, `withdrawn`, `closed`), `category`, `group_id` (nullable FK to groups — replaces the prior `community_id`), `brand_label` (nullable; same `member_id` + same `brand_label` = locally owned multi-location; powers resolve-up rendering), `qr_card_url` (nullable text — populated by `item.qr_card.request` action handler), `made_at_place_id` (nullable FK to `places.id`; meaningful only when `kind='product'`; per ADR-21 — substrate for the "Locally Made" badge), `made_at_verification_source` (text default `'none'`; check `in ('none','self_attested','community_attested','document_supported')` — corrected via T064 2026-05-25; ADR-21 verification-ladder reshape supersedes the original three-value form), `ambient_extras jsonb`, reserved: `parent_item_id`, `collection_id`, `federation_origin`, `embedding_id`. Soft-delete, timestamps.
...
- `009i_item_events.sql` — Item event log, partitioned monthly, audit fields. Event kinds at b1: `item.created`, `item.updated`, `item.published`, `item.location_attached`, `item.location_removed`, `item.responded`, `item.response_withdrawn`, `item.state_changed`, `item.fulfilled`, `item.deleted`, `item.group_changed`, `item.brand_label_changed`, `item.qr_card_requested`, `item.made_at_set` / `item.made_at_removed` / `item.made_at_verified` (per ADR-21 — reserved at b1; the "Locally Made" surface is deferred pending verification-ladder design). Reserved at b1, surfaces at T2: `item.converted` (Wonder → Gathering / Initiative), `item.collaborator_added`.
...
- **Onboarding suggestion step** — when a Member sets a `primary_home` Place, surface listed Groups whose `anchor_location_id` resolves to a Location inside that Place (or a child Place) with tap-to-join. Skippable; defaults to no memberships.
- **Concerts in the Park surface** (per canonical example #12) — per ADR-21, served two ways at b1: (a) the community-awareness feed in `discovery.md`, which reads `member_place_interests` × `member_interests` × Place-hierarchy traversal automatically (no explicit follow needed); (b) the `member_saved_searches` substrate, which lets a Member compose narrower filters (e.g., `place_id=Capitol Park's Place`, `interest_tags=['outdoor','live-music']`). The explicit saved-search composer surface and the fan-out worker ship at b2; the awareness feed reads the substrate at b1.
...
- **Phase 1 — DONE 2026-05-19 + b1.x extension DONE 2026-05-25.** All schema, RLS, evals shipped: Members + Locations + Groups + Items + discoverable_items (Phase 1 core, T041–T057); plus Places primitive + ADR-21 substrates (`member_place_interests`, `member_saved_searches`, `member_business_jurisdictions`) + ADR-22/23 URL refinements (b1.x extension, T058–T066). Eval state: **191/191 Phase 1 green** (107/107 unit + 191/191 Playwright evals); action-layer conformance 0 violations across 125 files / 32 protected tables. Migration sequence (dependency-driven, deviated from plan): 007 locations → 008 locations RLS → 009 members_phase1 → 010 member interests/follows → 011 affinities → 012 agent assistance → 013 delegations CHECK → 014 groups → 015 items → 016 discoverable_items (Phase 1 core); 017–022 places + ADR-21 substrate tables (b1.x). Actual shipped migration sequence is authoritative in BUILD-LOG + DEVIATIONS; the plan's 001–016 numbering is descriptive. Two going-forward rules added to DEVIATIONS at T055/T057 close: (1) SECURITY DEFINER pattern for cross-table RLS recursion; (2) `eval_indexes_for_table` returns `pg_indexes` column names (`indexname`, `indexdef`). Action handlers for Locations / Groups / Items intentionally NOT shipped in Phase 1 — they land with the Phase 2 surface composers that need them.
