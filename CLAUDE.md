...
- **Active Phase 2 plan:** none currently ratified. The prior sequence draft was retired 2026-05-28 (`_attic/2026-05-28-superseded-sequence-draft/`); a fresh draft sits in `_inbox/b1-primitives-sequence.md` awaiting triage. Scenario files live in [`planning/scenarios-backlog/`](planning/scenarios-backlog/).
- **Phase 3 plan:** not yet drafted. Phase 3 scenarios (`/explore` anonymous, Wonder if it returns to b1, `/why` thesis page, Group browse/create, stewardships) await a separate strategy pass.
...
| Dated work product | `_attic/YYYY-MM-DD-{slug}/` directly (per ADR-25 + the 2026-05-30 retirement of `housekeeping/` — see [`playbooks/DEVELOPMENT-PATTERNS.md`](playbooks/DEVELOPMENT-PATTERNS.md) § Route work through `_inbox/` → `planning/` kanban → `playbooks/`) | `_attic/2026-MM-DD-some-effort/` | `_attic/` |
...
6. **Archives split by kind.** *Retired specs* and *shipped-version release docs* go to directory-local archives per [ADR-25](planning/adrs/ADR-0025-local-lifecycle-ownership.md) — `{owning-dir}/archive/YYYY-MM-DD-{slug}/`; shipped-version cuts wrap each `{owning-dir}/archive/` into `{owning-dir}/archive/vN-{slug}/`, then reset for the next version. *Dated work-products* (atomized inboxes, retired top-level dirs, one-off audits) go to `_attic/YYYY-MM-DD-{slug}/` directly (per the 2026-05-30 retirement of `housekeeping/` — see [`playbooks/DEVELOPMENT-PATTERNS.md`](playbooks/DEVELOPMENT-PATTERNS.md) § Route work through `_inbox/` → `planning/` kanban → `playbooks/`). Provenance lives in frontmatter (`retired_from:` for retired specs; `RETIRED.md` for dated archives). Pre-2026-05-28 entries under `_attic/YYYY-MM-DD/{original-path}` are grandfathered; do not retroactively migrate. Root `planning/RELEASES.md` cross-indexes shipped versions across directories.
