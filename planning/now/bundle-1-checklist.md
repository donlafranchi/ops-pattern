...
- [ ] ~~Adjust how wide their "near me" reach is~~ — *out of v1 (F031). Metro-level only; distance removal (F048) deletes the width there was to adjust.*
...
- **Newcomer surfaces:** signup → locality feed (F030) and *Member page + follow* (F032) are done (evals green, merged). The unified *follow everything* page (F042) is **done** — built + merged (T108/T109). *Adjust near-me reach* (F031) is **out of v1** — metro-level only.
- **Integration test:** waits on the two end-to-end journeys, now gated on the v1 workstream list rather than F031.
