...
| Shell-entity smell | **clean** | F036 creates a `Group` of People, not a Business entity (per [`primitives.md`](../../product/foundation/primitives.md) § Why no Business entity). Items will FK to Members (`items.member_id NOT NULL` per groups.md:344); `groups.founder_member_id` is a Member FK; no business-as-owner column anywhere. |
...
2. **"Shop" vs "business Group" in UI copy.** **Ratified 2026-06-01: "Shop" is the UI label for kind='business' Groups, per the CLAUDE.md naming table.** T073 lands user-facing strings as "Shop" throughout — walkthrough title ("Set up your Shop"), success state ("Your Shop is live"), `/you` empty state ("You don't have a Shop yet"), page chrome on `/p/.../g/[slug]`. Schema (`groups.kind='business'`), URL (`/g/[slug]`), and CTA ("Sell") all unchanged. Scenario body still reads "business Group" — patch in a `tidy` pass after F036 ships; not blocking.
