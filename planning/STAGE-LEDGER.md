...
| F035 | Viewer finds Group public page | `tickets` 2026-06-02 (built; eval-run pending) | groups.md, business-jurisdiction.md | 2026-05-28 → scenario drafted 2026-06-01 (`scenario-F035-rosa-finds-mayas-shop.md`; Gate A PASS) | moved → `planning/now/` 2026-06-02 (direct build per PM) | covered by `review-F036.md` (sibling `/p/.../g/[slug]` surface); no separate `review-F035.md` | pending (Playwright F035 spec — `test`-skill step) | T074 (✅ build complete, branch `t074`) | pending | Anchor: use-cases.md P1 (primary), C1 (secondary). Business-Group flavor built; community-Group flavor (Run Club) deferred to same F-number, different slug, after F034. T074 ships beats 1/3/5/6 fully; Beat 2 badge render-path-only (F037/S-jurisdictions forward-dep); Beat 4 follow persistence deferred to F042 (no group-follow substrate — see SPEC-PATCHES). Greens `:167` in F036's eval suite. |
...
| F037 | Producer claims Locally Owned (Tier 0) | `plan-backlog` | business-jurisdiction.md; ADR-21 | 2026-05-28 → scenario drafted 2026-06-01 (`scenario-F037-maya-claims-locally-owned.md`; Gate A PASS) | — | — | — | — | — | Anchor: use-cases.md P4 (jurisdiction half). **S-jurisdictions substrate gate** prevents promotion to `next/`. Greens the jurisdiction half of `:266` once substrate + this surface ship. |
...
| F039 | Producer claims Locally Made (Tier 0) | `plan-backlog` | item.md Provenance, places.md; ADR-21 | 2026-05-28 → scenario drafted 2026-06-01 (`scenario-F039-maya-claims-locally-made.md`; Gate A PASS pending `item.md` § Provenance re-check at substrate ship) | — | — | — | — | — | Anchor: use-cases.md P4 (provenance half). **Two substrate gates**: S-jurisdictions (`made_at_place_id` column + `places` seed) AND F038 (product composer — the surface F039's field lives on). Cannot promote until both ship. Greens the provenance half of `:266` once all three land. |
