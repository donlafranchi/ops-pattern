...
| F032 | Viewer finds Member page and follows | `eval` GREEN 2026-06-02 (built on `t-f032`, **not merged**) | member.md, member-profile.md | 2026-05-28 → moved → `planning/now/` 2026-06-02 (PM-directed) | — (no separate review-F032; PM-directed direct build, mirroring F034/F035) | written 2026-06-02 (`web/evals/features/F032-…spec.ts` + fixture `evals/fixtures/F032-member-page.ts`) | T091 (follow/unfollow handlers + server actions) · T092 (Member page + resolver + component + eval) — 2026-06-02 | **GREEN 2026-06-02 — 6/6 pass** (run-mode, live local Supabase) | Anchor: use-cases.md C1 (viewer side). Replaces archived F025 (Member-page half). Substrate (`member_follows`, event kinds) pre-existed (T048); F032 is surface + handlers. **member→member follow lights up end-to-end** (F035's deferred group-follow is F042). New additive migration `029` (grant standing-presence view to anon + public `member_public_group_memberships` projection) — DEVIATIONS + SPEC-PATCHES filed. |
