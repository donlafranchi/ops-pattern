...
### Amended 2026-09-04 — v1 filters by metro, and this entry drops to the versioned tier

**PM directive, taken at F059 merge scope.** *"We are filtering by metro now. These are the kind of decisions that need to be removed from global decisions and entered into this version's decision matrix."*

Two separate moves, both deliberate:

1. **For v1, the feed filters.** The active metro is a boundary, not a ranking band. Items outside it are absent, not ranked-last. The "nothing is excluded from the catalog" claim above **does not describe v1** and must not be cited as though it does.
2. **The entry itself is demoted.** "Rank, never filter" was written as a global, indefinite commitment. It is a **bet** — falsifiable, cheap to reverse, and about product feel at a particular inventory density. It belongs in the version's decision matrix, not in the standing set. Per [`decision-durability-register.md`](decision-durability-register.md) § 9a, the constitutional tier stays narrow precisely so that strong early stances like this one can be taken and dropped without a memo.

**What stays true.** The *reasoning* for rank-over-filter is sound at scale and worth returning to: a feed that hides rows reads as a dead platform, and a filter the Member cannot see is one they cannot undo. It is the right shape once a metro holds enough inventory that the far band is a scroll rather than the whole catalog. **Revisit trigger:** a seeded metro exceeds a few hundred published Items, or a Member has a reason to look past the metro boundary that the switcher does not already serve.
