...
|---|---|---|---|---|
| **S-jurisdictions** — `member_business_jurisdictions` Tier 0 + `zip_is_proximal_to_location()` + handlers | [`T075-member-business-jurisdictions-substrate.md`](../../development/tickets/T075-member-business-jurisdictions-substrate.md) | ☐ open (buildable now — all deps T055/T070/T060 done) | F037, F039; jurisdiction half of F036 `:266` | T075 retained for this ticket |
| **Polygon/centroid seed** — Sacramento-region `places` polygons + `centroid` column | [`T076-places-polygon-centroid-seed.md`](../../development/tickets/T076-places-polygon-centroid-seed.md) | ☐ open (buildable now — deps T058/T066/T059 done) | polygon half of F036 `:266` locality step | ✅ renumbered T075→T076 (2026-06-02); migrations to rebase off `024_*` at build. **Per D3 (2026-06-02): seeds county/city/neighborhood tree rows only — no metro/region row; the Sacramento metro polygon moved to S-metro (CSA grain).** |
...
| **S-metro** — `metro_polygons` table + Census CSA seed + `members.home_metro_id` | — (not yet ticketed) | ☐ next — needs ticket | F031 | Unblocked: metro-polygon overlay **D1 (overlay) + D2 (CSA grain) + D3 (polygons own metros, not region rows) + D4 (isochrone deferred) all ratified 2026-06-02** in PLATFORM-PATTERNS. Table not built; needs a ticket. **Owns the Sacramento metro geometry** (CSA grain, wider than the four-county MSA) — the region-row metro seed was dropped from T076 per D3 and moved here. |

> ✅ **Ticket-number collision RESOLVED (2026-06-02).** The polygon-seed ticket was renumbered `T075` → **T076** (`T076-places-polygon-centroid-seed.md`); `T075` now uniquely names the jurisdictions substrate. Both still draft a `024_*` migration — the second to build rebases its migration to `025_*`+ at build time.
...
- [x] **F038 — Producer lists a product** · status: **build complete (eval pending)** · deps: F036 ✅ · gate: — · *Built 2026-06-02 on branch `t77` (T077 handlers `5a23f70` + T078 composer `845e354` + T079 Item page `2ea68a3`). Product composer end-to-end + skip-path provenance + place-scoped Item URL. Greens part of F036 `:224`. Unblocks F039 + F041. **Awaits PM merge of `t77`→main, then `test` Playwright eval.** Scenario moved backlog→now; tickets T077–T079 written this build (no prior scope/review — PM-directed direct build, mirroring F035).*
