...
| **Polygon/centroid seed** — Sacramento-region `places` polygons + `centroid` column | [`T076-places-polygon-centroid-seed.md`](../../development/tickets/T076-places-polygon-centroid-seed.md) | ☐ open (buildable now — deps T058/T066/T059 done) | polygon half of F036 `:266` locality step | ✅ renumbered T075→T076 (2026-06-02); migrations to rebase off `024_*` at build |
...
| **S-metro** — `metro_polygons` table + Census CSA seed + `members.home_metro_id` | — (not yet ticketed) | ☐ next — needs ticket | F031 | Unblocked: ADR-24 D1 (overlay) + D2 (CSA grain) + D4 (isochrone deferred) ratified 2026-06-02 in PLATFORM-PATTERNS. Table not built; needs a ticket. (D3 region-kind-vs-polygon ownership still open — does not block building the overlay table.) |
