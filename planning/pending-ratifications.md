...
| Person<->Location affinity kinds | `product/systems/member.md`, `location.md` | lives, works, plays, visits, follows, liked | SOFTEN — redesign | 2026-05-23 — the six-kind model is superseded. PM reframe: (1) `lives`/`works` are **seller-only** locality declarations feeding the "Support Local" badge — they leave `member_location_affinities` for the business-jurisdiction subsystem; (2) `plays`/`visits`/`liked` dropped; (3) community awareness = **Place + interests** computed feed, not raw Location-follow (a Location is too coarse — the alert algorithm filters by interest). `member_location_affinities` as a six-kind table largely dissolves. Routed to a `pipeline-product` exploration — not ratifiable as a walkthrough row. Entangles #22 (ADR-16 on `member_location_affinities`). **PM note 2026-05-23:** "Locally Made" (product provenance — where the product is made) is a distinct claim from "Locally Owned" (owner residence) — the redesign should treat them as separate seller badges with separate verification stories. |
...
| ADR-16 per-row privacy | `product/systems/member.md` | RLS owner-only on `member_location_affinities` | DEFER | 2026-05-23 — privacy *intent* (owner-only RLS) confirmed and stable; it carries over to whatever `member_location_affinities` becomes. The implementation deep-dive rides with the #19 member↔geography redesign, since that redesign reshapes the very table this ADR governs. Review as part of the redesign. |
