...
|---|---|---|---|---|
| Group kinds at b1 | `product/systems/groups.md`, ADR-13 | place, interest, practice, event_anchored, family, business | KEEP | Ratified 2026-05-23 — six kinds upheld. **Honest finding:** only `business` is feature-bearing across the board; `event_anchored` diverges in one cell (its child table); `place`/`interest`/`practice`/`family` drive zero distinct features at b1 — kept as a discovery facet + divergence seam. **State tag: Ratified 2026-05-23; affiliate-flavor collapse deferred — review at b2 entry** (collapse to a descriptive tag if the flavors still drive no distinct features). `congregation` deferred — modeled as `practice`/`interest`; enum is extensible. Intent line queued for `groups.md` (pending the in-flight CC session on that file). |
| Person<->Location affinity kinds | `product/systems/member.md`, `location.md` | lives, works, plays, visits, follows, liked | **RATIFIED — RETIRED** | 2026-05-23 — RATIFIED via [ADR-0021](adrs/ADR-0021-member-geography-substrate-split.md). The six-kind enum and `member_location_affinities` table dissolve. Three purpose-owned substrates land: `member_business_jurisdictions` (seller locality, existing) + `items.made_at_place_id` (new "Locally Made" product provenance) + `member_place_interests` (new, community awareness, Place-shaped, private) + `member_saved_searches` (new, generalized follow/interest-in-a-Place, absorbs "follow this venue"). `plays`/`visits`/`liked` dropped — no consumer surface ever earned them. `lives`/`works` exit the schema entirely — jurisdiction is the only public locality-derivation path. Spec patches landed across `member.md`, `location.md`, `places.md`, `business-jurisdiction.md`, `groups.md`, `discovery.md`, `item.md`. Source exploration: `product/exploration/member-geography-redesign.md`. |
| Location kinds | `product/systems/location.md`, ADR-14 | permanent, recurring_temporary, area | KEEP | Ratified 2026-05-23 — all three kinds earn their slots: each carries a distinct child table (`location_permanent` / `location_recurring_temporary` / `location_areas`), distinct geometry semantics, and distinct handlers. A feature-bearing enum — passes the divergence test cleanly, unlike Group affiliate-kinds (#18) and affinity-kinds (#19). Intent line landed in `location.md`. |
| Standing-tier gate | `product/systems/groups.md` | >=1 active membership in kind='business' Group OR steward-role membership in any non-business Group | DEFER | 2026-05-23 — deferred to b2 entry. The gate's only consumers are b2 agent-assistance surfaces (Assistant Context prominence, agent context tier, Skill-subscription affordances); b1 ships substrate only. Ratifying the precise bar now is premature, and the gate reads Group/membership state the #19 redesign is reshaping. State tag: Deferred until b2 entry — review when the agent-assistance surfaces are built. Intent line queued for `groups.md` (in-flight CC session). |
| ADR-16 per-row privacy | `product/systems/member.md` | RLS owner-only on `member_location_affinities` | **PARTIALLY SUPERSEDED by ADR-21** 2026-05-23 | The privacy *posture* (owner-only RLS, named SECURITY DEFINER access patterns, service-role bypass for backend pipelines) is ratified and carries forward — applied now to `member_place_interests` and `member_saved_searches` per [ADR-0021](adrs/ADR-0021-member-geography-substrate-split.md). The original target table (`member_location_affinities`) and its three named functions (`count_likes_for_location`, `count_followers_for_location`, `member_is_local_to_location`) retire because no consumer survives the split. |
