...
**Touches:** `product/systems/member.md` (RLS section + Decisions-encoded header), `product/systems/groups.md` (locally-owned derivation), `product/foundation/policy-framework.md` (anti-doxxing — ADR-9 scope), `product/systems/business-jurisdiction.md` (Tier 0 ZIP verification follows the same pattern), `web/supabase/migrations/` (RLS policies + the three SECURITY DEFINER functions)
...
5. [x] `policy-framework.md` anti-doxxing section upgraded.
