...
**Companions:** [`bundle-themes.md`](bundle-themes.md) sequences b1 into seven sub-bundles (`b1.0`–`b1.6`, plus cross-cutting `b1.x`). [`b1-primitives-work-map.md`](b1-primitives-work-map.md) lists the menu of work per sub-bundle, tagged 🟢 / 🟡 / ⚪. Read all three to scope an F### scenario.
...
- **Stewardship rotation, pooled-capital tooling, capital-flow surfaces.** Loop 11 / Loop 12 surfaces. Substrate intent at b1 (per `groups.md` lifecycle + Group event log); user-facing surfaces are b2/b3. **Cooperative-style coordination is deferred until real-world need + explicit user prioritization** — no `cooperative_*` schema lands at b1.
...
- Item — **spine + kind-specific child tables** (the four kinds share a logical primitive but have separate physical tables for typed columns and per-kind indexes; see `item.md` for the full rationale):
  ...
  - **No `cooperative_cohort` value in the `items.kind` enum.** Cooperative-style coordination is deferred until real-world need + explicit user prioritization. No `cooperatives`, `cooperative_assets`, `item_cooperative_cohorts`, `cooperative_distributions`, or `cooperative_governance_votes` tables ship at b1.
...
- `service_providers.availability_kind`, `service_providers.verified_badges` (no `cooperative_id` — cooperative coordination deferred until real-world need; the cooperative-shape use case is served at b1 by kind='business' Groups with multiple owner-role memberships)
...
Sequencing lives in [`bundle-themes.md`](bundle-themes.md) — b1 ships in seven sub-bundles (`b1.x` URL namespacing → `b1.0` through `b1.6`), each 1–2 weeks. The menu of work per sub-bundle, tagged 🟢 / 🟡 / ⚪ for scope decisions, lives in [`b1-primitives-work-map.md`](b1-primitives-work-map.md). The planner reads both before writing the next F### scenario.
...
- **Initial tag vocabulary per Item kind.** Curated before the first Item is created. For gathering Items specifically, a controlled list of gathering categories is sized as part of this open question.
