...
| Policy posture present | n/a — no new surface | F018 publishes an Item (public-by-default at `state='published'`), attaches to a Location (`discoverability='listed'` default), and surfaces in the locality-first index. No new policy posture is introduced; the existing posture in `item.md` / `location.md` / `policy-framework.md` covers this. ✓ |
