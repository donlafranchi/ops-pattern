...
- [ ] 2026-06-02 · `product/systems/member.md` § Profile — F030 onboarding writes profile fields (display_name/handle/bio/pronouns/avatar) directly via the owner-update-RLS client with **no** `member.updated` event (profile edits are not declarations, unlike place-interest/interest which emit). There is no `member.update` action handler. Decide whether profile edits should be event-sourced through a `member.update` handler (emitting `member.updated`) or remain direct RLS writes; if the former, assign a substrate ticket. Caught by T089. DEVIATIONS: 2026-06-02 — T089.
- [ ] 2026-06-02 · `planning/now/scenario-F030-newcomer-signs-up-and-lands-in-feed.md` § Acceptance (AC1 "IP-geolocated locality") + § Surfaces — b1 ships a launch-locality default + scope picker instead of real IP geolocation (no geo provider wired). The scenario's own IP-fail edge case sanctions a picker fallback. Confirm the scenario should mark IP geolocation as a b2 refinement, and that the feed's Member-scoped Item URLs (`/m/<handle>/<seg>/<slug>-<id8>`) — rather than Group place-path URLs — are acceptable at b1. Caught by T088. DEVIATIONS: 2026-06-02 — T088.
