...
- Each ticket is session-sized (~1–3 hours of build work). If a scenario produces 5+ tickets, the scenario is too big — escalate to `pipeline-plan` to split it.
...
**Next skill:** `pipeline-eval` (write mode) — translates the scenario's Given/When/Then into Playwright tests *before* the build agent starts. Then `pipeline-build` to implement the ticket via TDD.
...
## Related skills
- `pipeline-plan` — the upstream filter that produces approved scenarios.
- `pipeline-eval` — writes evals from the scenario, runs them after build.
- `pipeline-build` — implements the ticket via TDD.
