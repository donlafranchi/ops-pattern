...
**Next skill:** `scope` — converts your systems/capabilities into user-story-shaped scenarios and applies the 5 Deadly Sins filter.

**Pipeline-plan expects:**
- A system spec at `product/systems/{name}.md` with tiers (T1/T2/T3) and a Data model implications section.
- A canonical example in `product/needs/use-cases.md` that the system serves. If none fits, add one — scope will refuse to scenarioize a system with no canonical anchor.

## Related skills
- `scope` — downstream; filters and writes scenarios from your specs.
