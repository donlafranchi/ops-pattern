...
**Next skill:** `build` — implements the ticket via TDD. Build does not read your test file (no peeking at test internals); it writes its own unit tests against the ticket's checklist and lets your evals validate from the outside.
...
**Next skill (on fail):** `build` — fixes forward. Never roll back. Never silently update tests to match a wrong implementation.

**You DO NOT:** fix failing tests. Fix-forward is build's job.

## Related skills
- `scope` — upstream; produces approved scenarios you read from.
- `ticket` — sibling; reads same scenario you do, in parallel.
- `build` — downstream (write mode) or downstream-on-fail (run mode).
