...
name: atomize
description: Bridge between Cowork strategy and Claude Code execution. Reads a plan or parked-decision doc in `_inbox/`, decomposes it into ratify-and-execute stubs in `planning/backlog/`, archives the parent. Triggers on "atomize the inbox", "atomize `_inbox/{name}.md`", "decompose this plan", "break this plan into backlog items", "materialize the inbox", "intake the plan". Reads only `_inbox/`, `REGISTRY.md`, `CLAUDE.md`, the file-naming table, and the file the user pointed at. Writes only into `planning/backlog/` and archives the parent to `_attic/YYYY-MM-DD-{slug}/`. Does not ratify decisions, does not write scenarios, does not write tickets — produces the backlog stubs that PM ratifies and downstream skills (`weigh`, `scope`, `tidy`, `ticket`) pick up.
---

# atomize

Project-resident pipeline-front skill. Translates `_inbox/` plans and parked decisions into atomic, ratify-and-execute stubs in `planning/backlog/` — the lane between untriaged drafts and PM-ratified queued work.
...
- A single parked decision sits in `_inbox/` that needs a backlog-lane stub so PM can ratify it through `weigh` or route it to a system spec.
...
- An already-approved scenario sits in `planning/next/` or `planning/now/` and needs tickets — that's `ticket`'s job, not atomize.
- A ratified item sits in `planning/backlog/` waiting to move — PM moves it manually to `next/` or `now/`; atomize does not orchestrate lane progression.

## Constraints (hard)

- Read only `_inbox/`, the file the user pointed at, `REGISTRY.md`, root `CLAUDE.md` (for the file-naming table), and `planning/backlog/` (to assign sequence numbers if grouping).
- Write only into `planning/backlog/` and `_attic/YYYY-MM-DD-{slug}/` (when archiving the parent plan).
- Never read or write `web/` code, `development/tickets/`, `planning/next/`, `planning/now/`, or any system spec — atomize is a routing skill, not a content skill. The downstream verb skill produces the actual content.
...
- Do not ratify, do not execute, do not invoke the downstream skill. Atomize stops at backlog-lane handoff.
...
For a multi-item plan, atomize produces a sub-directory in `planning/backlog/` named after the parent plan, containing one stub per atom plus an index README:
...
**You produced:**
- One or more stubs in `planning/backlog/` (flat or grouped under `{plan-slug}/`).
...
- PM moves the file from `planning/backlog/` to `planning/next/` (queued) or `planning/now/` (in flight).
- PM invokes the `route:` skill, which picks up the file from its new lane.

**A fresh backlog stub is not parked work.** A stub older than ~2 weeks in `planning/backlog/` is a flag — either ratify it (move to `next/`/`now/`) or reject it. Atomize does not enforce this; `orient`'s drift check should.
...
- `orient` — surfaces stale entries in `planning/backlog/` during the drift check.
